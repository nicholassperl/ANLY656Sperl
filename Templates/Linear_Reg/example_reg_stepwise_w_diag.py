...
from   AdvancedAnalytics.Regression           import stepwise, linreg
...
y = encoded_df[target]
X = encoded_df[stepwise_selected]
...
RED = "\033[38;5;196m"  # Bright 256-color red
RESET = "\033[0m"
...
for row in table_data:
    ...
    if expected_float < 1 and actual > 0:
        ...
        actual_str = " " * spaces + f"{RED}{actual}{RESET}"
    else:
        # Add padding for normal values too
        spaces = 15 - len(str(actual))
        actual_str = " " * spaces + f"{actual}"
...
for n_folds in range(2, 6):
    lr = LinearRegression()
    scores  = cross_validate(lr, X[stepwise_selected], y, 
